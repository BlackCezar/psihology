...
@foreach ($news as $page) 
  <li class="list-group-item list-group-item-action d-flex flex-column">
  	<div class="align-items-center d-flex w-100 justify-content-between">
		<h4>{{ $page->title }}</h4>
  		<div class="btns">
		  <button class="btn btn-link" onclick="window.location = '/news/{{ $page->id }}'">Перейти</button>
  	...
  	<form class="d-none" action="/admin/pages" method="post">
  		<div class="form-group">
			...
			<input type="text" class="form-control" aria-describedby="title" name="title" value="{{ $page->title }}">
	  	</div>
	  	<div class="form-group">
			<label for="exampleInputEmail1">Контент</label>
			<textarea class="form-control" name="body" rows="10" aria-describedby="body">{{ $page->body }}</textarea>
	  	</div>
